NEWSCHEMA('Product').make(function(schema) {
	...
	schema.define('price', Number, false);
	...
	schema.setSave(function(error, model, options, callback, controller) {
		...
		model.linker = model.id;
		// model.linker = ((model.reference ? model.reference + '-' : '') + model.name).my_slug();
		model.linker_manufacturer = model.manufacturer ? model.manufacturer.my_slug() : '';
...
function prepare_subcategories(name) {
	...
	for (var i = 0, length = category.length; i < length; i++) {
		...
		builder_link.push(item.my_slug());
...
String.prototype.my_slug = function(max) {
    max = max || 60;
    ...
    for (var i = 0; i < length; i++) {
        ...
        if ((code > 47 && code < 58) || (code > 94 && code < 123))
            builder += c;
        else builder += Math.floor(Math.random() * 10);
}
