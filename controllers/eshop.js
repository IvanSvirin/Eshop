...
function view_products_detail(linker) {
    ...
    self.$read(options, function (err, data) {
        ...
        // Renders view
        if (data.category === 'Изделия') {
            self.view('~cms/' + ('constructor'), data);
        } else {
...
function view_checkout(linker) {
    ...
    self.$read(options, function (err, data) {
        ...
        if (self.query.pay === '0') {
            self.view('checkout-gift');

            // var mailgun = require("mailgun-js");
            // var api_key = 'key-1623fe8375134bf8a756ec6bba9fefe9';
            // var DOMAIN = 'knittedfabric.ru';
            // var mailgun = require('mailgun-js')({apiKey: api_key, domain: DOMAIN});
            // for (var i = 0, length = data.products.length; i < length; i++) {
            //     var product_pictures = data.products[i].pictures;
            //     var picture_array = [];
            //     picture_array = (product_pictures + "").split(',');
            //     var paths_array = [];
            //     for (var j = 0, pictures_qty = picture_array.length; j < pictures_qty; j++) {
            //         paths_array[j] = 'knittedfabric.ru/download/' + picture_array[j] + '.jpg';
            //     }
            //     var data = {
            //         from: 'Excited User <me@samples.mailgun.org>',
            //         to: data.email,
            //         subject: paths_array[0],
            //         text: paths_array
            //     };
            ...
            let transporter = nodemailer.createTransport({
                host: 'knittedfabric.ru',
                port: 587,
                secure: false, // true for 465, false for other ports
                auth: {
                    user: 'root@knittedfabriq.ru',
                    pass: 'i12472'
                }
            });
            for (var i = 0, length = data.products.length; i < length; i++) {
                ...
                let mailOptions = {
                    // from: 'ivan_svirin@mail.ru',
                    to: data.email,
                    subject: data.products[i].name,
                    // text: paths_array[0],
                    attachments: [
                        // {
                        //     path: paths_array[0]
                ...
                var paths_array = [];
                for (var j = 0, pictures_qty = picture_array.length; j < pictures_qty; j++) {
                    paths_array[j] = 'http://knittedfabric.ru/download/' + picture_array[j] + '.jpg';
                    mailOptions.attachments.push({path: paths_array[j]});
                }
